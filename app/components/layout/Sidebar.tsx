...
const Sidebar = ({ isOpen, toggleSidebar }: SidebarProps) => {
  ...
  const userLinks = [
    ...
    { to: "/user/rooms", icon: <Bed size={20} />, label: "View Rooms" },
    { to: "/user/book", icon: <Calendar size={20} />, label: "Book Room" },
    { to: "/user/bookings", icon: <BookOpen size={20} />, label: "My Bookings" },
  ]

  const adminLinks = [
    { to: "/admin", icon: <Home size={20} />, label: "Dashboard" },
    { to: "/admin/hotels", icon: <Hotel size={20} />, label: "Manage Hotels" },
    { to: "/admin/rooms", icon: <Bed size={20} />, label: "Manage Rooms" },
  ...
  return (
    <>
      ...
      <motion.aside
        ...
        className={`fixed md:relative w-64 h-full bg-indigo-700 text-white z-30 md:translate-x-0 shadow-lg`}
      >
            ...
            <ul className="space-y-1 px-2">
              {links.map((link) => (
                <li key={link.to}>
                  <NavLink
                    to={link.to}
                    end={link.to === "/user" || link.to === "/admin"}
                    className={({ isActive }) =>
